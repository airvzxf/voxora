...
| T2.5 | <2 min | pre-push / CI | `cargo bench --workspace --no-run` (compile-only criterion harness lane — closes #56). Every `[[bench]]` target across the workspace compiles; the actual measurements stay `#[ignore]`-gated because they need real model weights. |
| T3 | CI | CI | `cargo build --workspace --locked` + `RUSTUP_TOOLCHAIN=1.88 cargo check --workspace --locked` (closes #92; declared MSRV per workspace `rust-version`) + `cargo deny check` + `.github/workflows/quality-nightly.yml` (nightly `cargo fuzz` for 60 s against each parser target — closes #54, EPIC #133 fuzz infrastructure; nightly-only so PR throughput stays unaffected). |
