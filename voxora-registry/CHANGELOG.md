...
## [0.6.3](https://github.com/airvzxf/voxora/compare/voxora-registry-v0.6.2...voxora-registry-v0.6.3) - 2026-09-23

### Other

- *(release)* coordinated 0.6.3 patch bump ([#243](https://github.com/airvzxf/voxora/pull/243))

## [0.6.2](https://github.com/airvzxf/voxora/compare/voxora-registry-v0.6.1...voxora-registry-v0.6.2) - 2026-09-21

### Other

- updated the following local packages: voxora-traits, voxora-hf, voxora-local, voxora-engine
...
### Added
- **`benches/resolve.rs`** (criterion, dev-only, compile-checked in
  CI): a stub harness that exercises the central
  `Registry::with_builtin_descriptors()` resolver path. `#[ignore]`-gated
  so the workspace-wide `cargo bench --workspace --no-run` lane
  that closes #56 stays green on CI without network access;
  running the bench requires model weights.
